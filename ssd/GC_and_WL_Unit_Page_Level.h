...
namespace SSD_Components
{
	...
	public:
		...
		* GC should go on in non-preemptible mode.*/
		...
		void Check_data_migration_required(const unsigned int free_slc_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address);
		void Check_tlc_gc_required(const unsigned int free_slc_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address);
		void Try_check_data_migration_required();
		Data_Migration_Policy dm_policy = Data_Migration_Policy::QPC_DM;
		int Consecutive_dm_num = 0;
	private:
